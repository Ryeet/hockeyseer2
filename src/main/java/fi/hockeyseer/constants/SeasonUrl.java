package fi.hockeyseer.constants;

/**
 * Created by alekstu on 5.6.2017.
 */
public final class SeasonUrl {


    public static final String s2016_2017 = "startDate=2016-10-12&endDate=2017-04-11";

    public static final String s2015_2016 = "startDate=2015-10-07&endDate=2016-04-11";

    public static final String s2014_2015 = "startDate=2014-10-08&endDate=2015-04-13";

    public static final String s2013_2014 = "startDate=2013-10-01&endDate=2014-04-15";


}
